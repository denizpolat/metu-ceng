...
namespace fst
{
    void Scene::loadFromParser(const parser::Scene& parser)
    {
        ...
        for (auto& mesh : parser.meshes)
        {
            ...
            meshes.push_back(Mesh(std::move(triangles), mesh.material_id, std::move(mesh.transformations)));
        }
        ...
        for (auto& triangle : parser.triangles)
        {
            ...
            meshes.push_back(Mesh(std::move(triangles), triangle.material_id, std::move(triangle.transformations)));
        }

        for (auto& sphere : parser.spheres)
        {
            spheres.push_back(Sphere(vertex_data[sphere.center_vertex_id - 1],
                sphere.radius, sphere.material_id, sphere.transformations));
        }
